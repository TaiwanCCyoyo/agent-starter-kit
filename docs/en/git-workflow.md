...
- **Current state: PR delivery enabled for `TaiwanCCyoyo/agent-starter-kit` only**, using `https://github.com/TaiwanCCyoyo/agent-starter-kit.git`, base `main`, and `taiwanccyoyo-dev-agent[bot]`. The owner authorizes daily independent review and conditional merge by `taiwanccyoyo-review-agent[bot]` as described in [daily review](daily-review.md). Reset this paragraph to local-only when adopting the template elsewhere.
...
- Agent-authored commits use the development bot name and its verified GitHub noreply email, scoped to the commit command; do not change the owner's global Git identity or rewrite existing commits solely to change attribution.
...
- Development and review roles must not directly push the default branch, bypass quality gates, or change repository rules. Merging requires separate owner authorization and must go through a PR. The configured reviewer may use its owner-granted PR-only exception to the separate merge-identity ruleset; this is not an exception to required reviews or checks.
