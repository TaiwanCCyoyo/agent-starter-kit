...
1. After opening the PR, wait for initial CI and the configured review. Check at bounded intervals; report meaningful changes rather than unchanged polls. If review does not start or an external blocker prevents progress, report the exact pending state instead of claiming completion.
