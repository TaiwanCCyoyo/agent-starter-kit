...
Antigravity accesses `.memories/memory_store.db` through the `memory-db` SQLite MCP server.
**IMPORTANT**: Due to current platform requirements, the `memory-db` MCP server must be configured globally in your home directory (e.g., `~/.mcp.json` or equivalent global config), rather than a project-local config.
