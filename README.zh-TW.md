...
2. **會進化的提交防護 (Evolving Pre-Commit)**：每次 `git commit` 前，Agent 必須嚴格執行 `PRE_COMMIT_SOP.md`。第一條永遠是強制性的金鑰掃描（如 `gitleaks`）；隨著專案加入新語言或工具，Agent 有責任主動將新的 Lint 與測試檢查更新入該 SOP 中。
...
* **`.agents/TEAM.md`**：專案子角色清單，供主 Agent 分配資源使用。
