...
- The main session owns the canonical document, normally English. After editing it, delegate one explicit non-canonical target document to `doc-translator` with a source diff or concise change list. If language versions disagree, synchronize the target to the main-session-maintained canonical document.
