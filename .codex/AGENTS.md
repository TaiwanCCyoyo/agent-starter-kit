...
- The main agent owns the canonical document, normally English. After editing it, delegate one explicit non-canonical target document to `doc_translator` with a source diff or concise change list. If language versions disagree, synchronize the target to the main-agent-maintained canonical document.
