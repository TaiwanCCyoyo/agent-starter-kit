...
class DismissReminderTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.state_path = self.root / ".memories" / ".codex_stop_memory_state.json"

    def tearDown(self) -> None:
        self._tmpdir.cleanup()

    def _load_server(self) -> ModuleType:
        server_path = ROOT / ".codex" / "mcp" / "memory_control.py"
        with patch.dict("os.environ", {"CODEX_PROJECT_DIR": str(self.root)}):
            return load_module("codex_memory_control", server_path)
